...
std::shared_ptr<LevelState> MapGenerator::GenerateMap(int levelIndex, int width, int height, const int MinLeafSize, const int MaxLeafSize, const int MinRoomSize)
{
	...
	mapGenerator.addDecorator(std::make_unique<EnemyDecorator>(7, 0, "Phantom", levelState, levelIndex));
	...
	mapGenerator.addDecorator(std::make_unique<ItemDecorator>(1, false, std::make_shared<Torch>(10, 12, levelIndex), levelState, levelIndex));
